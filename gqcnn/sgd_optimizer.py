...
class SGDOptimizer(object):
    ...
    def optimize(self):
        ...
        try:
            ...
            for step in training_range:
                ...
                if self.save_histograms:
                    self.summary_writer.add_summary(self.sess.run([self.merged_histogram_summaries], feed_dict={
                                                    self.gqcnn.input_im_node: train_images, self.gqcnn.input_pose_node: pose_node}))
    ...
    def _read_gripper_data(self, gripper_param_arr, input_gripper_mode):
        ...
        if input_gripper_mode == InputGripperMode.WIDTH:
            ...
            raise ValueError(
                'Input gripper mode {} not supportd'.format(input_gripper_mode))
    ...
    def _compute_data_metrics(self):
        ...
        if self.cfg['fine_tune']:
            ...
            for k in random_file_indices.tolist():
                ...
                self.data_std += np.sum(
                    (im_data[self.train_index_map[im_filename], :, :, :] - self.data_mean)**2)
        ...
        if self.cfg['fine_tune']:
            ...
            for k in random_file_indices.tolist():
                ...
                self.pose_data = np.load(os.path.join(
                    self.data_dir, pose_filename))['arr_0']
                if self.input_pose_mode == InputPoseMode.TF_IMAGE_SUCTION:
                    rand_indices = np.random.choice(
                        self.pose_data.shape[0], size=self.pose_data.shape[0] / 2, replace=False)
        ...
        if self.gripper_dim > 0:
            ...
            self.gripper_mean_filename = os.path.join(
                self.experiment_dir, 'gripper_mean.npy')
            self.gripper_std_filename = os.path.join(
                self.experiment_dir, 'gripper_std.npy')
            if self.cfg['fine_tune']:
                ...
                for k in random_file_indices.tolist():
                    ...
                    self.gripper_data = np.load(os.path.join(
                        self.data_dir, gripper_filename))['arr_0']
        ...
        if self.input_gripper_mode == InputGripperMode.DEPTH_MASK:
            ...
            if self.cfg['fine_tune']:
                ...
                self.gripper_depth_mask_mean = np.zeros((2)) # one for each channel of mask(fingertip/palm)
                self.gripper_depth_mask_std = np.zeros((2))
                random_file_indices = np.random.choice(
                    self.num_files, size=self.num_random_files, replace=False)
                num_summed = 0
                for k in random_file_indices.tolist():
                    ...
                    self.gripper_depth_mask_mean[0] += np.sum(
                        mask_data[self.train_index_map[im_filename], :, :, 0])
                    num_summed += mask_data[self.train_index_map[im_filename], :, :, 0].shape[0]
                    self.gripper_depth_mask_mean[1] += np.sum(
                        mask_data[self.train_index_map[im_filename], :, :, 1])
                    num_summed += mask_data[self.train_index_map[im_filename], :, :, 1].shape[0]
                self.gripper_depth_mask_mean = self.gripper_depth_mask_mean / \
                    (num_summed * self.im_height * self.im_width)
                ...
                np.save(gripper_depth_mask_mean_filename, self.gripper_depth_mask_mean)
                np.save(gripper_depth_mask_std_filename, self.gripper_depth_mask_std)
        ...
        if self.cfg['fine_tune']:
            ...
            out_gripper_mean_filename = os.path.join(
                self.experiment_dir, 'gripper_mean.npy')
            out_gripper_std_filename = os.path.join(
                self.experiment_dir, 'gripper_std.npy')
        ...
        self.gqcnn.update_pose_mean(self._read_pose_data(
            self.pose_mean, self.input_pose_mode))
        self.gqcnn.update_pose_std(self._read_pose_data(
            self.pose_std, self.input_pose_mode))

        if self.gripper_dim > 0:
            # update gqcnn gripper mean & std
            self.gqcnn.update_gripper_mean(self._read_gripper_data(
                self.gripper_mean, self.input_gripper_mode))
        ...
        if self.input_gripper_mode == InputGripperMode.DEPTH_MASK:
            ...
            self.gqcnn.update_gripper_depth_mask_std(
                self.gripper_depth_mask_std)
        ...
        for im_filename, metric_filename in zip(self.im_filenames, self.label_filenames):
            self.metric_data = np.load(os.path.join(
                self.data_dir, metric_filename))['arr_0']
        ...
        pct_pos_val = float(
            np.sum(all_val_metrics > self.metric_thresh)) / all_val_metrics.shape[0]
    ...
    def _read_data_params(self):
        ...
        self.train_im_data = np.load(os.path.join(
    ...
    def _setup_data_filenames(self):
        ...
        if self.image_mode == ImageMode.BINARY:
            self.im_filenames = [f for f in all_filenames if f.find(
        ...
        self.pose_filenames = [f for f in all_filenames if f.find(
        ...
        if self.debug:
            ...
            zipped = zip(self.im_filenames, self.pose_filenames, self.label_filenames, self.obj_id_filenames, 
                self.stable_pose_filenames, self.gripper_param_filenames, self.gripper_depth_mask_filenames, self.gripper_seg_mask_filenames)

            random.shuffle(zipped)
            zipped = zipped[:self.debug_num_files]

            # unpack
            self.im_filenames, self.pose_filenames, self.label_filenames, self.obj_id_filenames, self.stable_pose_filenames, self.gripper_param_filenames, self.gripper_depth_mask_filenames, self.gripper_seg_mask_filenames = zip(*zipped)
        ...
        while not self.term_event.is_set():
            ...
            while start_i < self.train_batch_size:
                ...
                if self.cfg['mask_and_inpaint']:
                    ...
                    train_data_arr, train_poses_arr = self._distort(train_data_arr, train_poses_arr, mask_and_inpaint=True)
                else:
                    train_data_arr, train_poses_arr = self._distort(train_data_arr, train_poses_arr)
                ...
                if self.training_mode == TrainingMode.REGRESSION:
                    if self.preproc_mode == PreprocMode.NORMALIZATION:
                        train_label_arr = (train_label_arr - self.min_metric) / (self.max_metric - self.min_metric)
                elif self.training_mode == TrainingMode.CLASSIFICATION:
...
#                    vis.show()
                ...
                if self.gripper_dim > 0:
                    train_gripper[start_i:end_i] = self._read_gripper_data(train_gripper_arr, self.input_gripper_mode)
    ...
    def _distort(self, im_arr, pose_arr=None, only_dropout=False, mask_and_inpaint=False):
        ...
        im_arr, pose_arr = copy.deepcopy(im_arr), copy.deepcopy(pose_arr)
    ...
    def _error_rate_in_batches(self):
        ...
        for data_filename, pose_filename, gripper_filename, gripper_depth_mask_filename, gripper_seg_mask_filename, label_filename in zip(self.im_filenames, self.pose_filenames, self.gripper_param_filenames, self.gripper_depth_mask_filenames, self.gripper_seg_mask_filenames, self.label_filenames):
            ...
            if self.cfg['distort_val_data']:
                data, poses = self._distort(data, poses)
