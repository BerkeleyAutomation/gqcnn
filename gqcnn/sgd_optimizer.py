...
from neon.optimizers import GradientDescentMomentum, ExpSchedule, StepSchedule
...
class SGDOptimizer(object):
	...
	def optimize(self):
		...
		# create optimizer
		self._optimizer = self._create_optimizer(self.base_lr, self.momentum_rate, self.train_l2_regularizer, self._learn_schedule)
		...
		self._train_model.fit(dataset=self._train_iter, cost=self._loss, optimizer=self._optimizer, num_epochs=self.num_epochs, callbacks=self._callbacks)
