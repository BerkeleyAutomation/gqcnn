"""
NervanaDataIterator for feeding validation data to GQ-CNN network for training.
...
class GQCNNValIterator(NervanaDataIterator):
    def __init__(self, im_filenames, pose_filenames, label_filenames, indices, train_config, im_mean, im_std, pose_mean, pose_std, make_onehot=True, nclass=2, name=None):
    ...
    def __iter__(self):
        ...
        for i in range(self.start, self.ndata, self.be.bsz):
            ...
            while start_ind_tensor < self.be.bsz:
                ...
                file_pose_data = self._read_pose_data(np.load(os.path.join(self.dataset_dir, self.pose_filenames[file_idx]))['arr_0'][indices], self.input_data_mode)
