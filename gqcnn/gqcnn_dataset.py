...
from gqcnn import GQCNNTrainIterator, GQCNNValIterator
...
class GQCNNDataset(Dataset):
    ...
    def gen_iterators(self):
        self._data_dict = {'train': GQCNNTrainIterator(self.im_filenames, self.pose_filenames, self.label_filenames, self.train_index_map, self.cfg, 
            self.data_mean, self.data_std, self.pose_mean, self.pose_std, name='train_data')}
        self._data_dict['val'] = GQCNNValIterator(self.im_filenames, self.pose_filenames, self.label_filenames, self.val_index_map, self.cfg,
