...
class DeepOptimizer(object):
    ...
    def _compute_data_metrics(self):
        ...
        if self.cfg['fine_tune']:
            ...
            for k in random_file_indices.tolist():
                ...
                self.pose_data = np.load(pose_filename)['arr_0']
        ...
        for im_filename, metric_filename in zip(self.im_filenames, self.label_filenames):
            self.metric_data = np.load(metric_filename)['arr_0']
    ...
    def _compute_indices(self):
        """ Compute train and validation indices """
        ...
        train_index_map_filename = os.path.join(self.experiment_dir, 'train_indices.pkl')
        self.val_index_map_filename = os.path.join(self.experiment_dir, 'val_indices.pkl')
    ...
    def _read_training_params(self):
        ...
        self.decay_step = self.cfg['decay_step']
    ...
    def _read_data_params(self):
        ...
        self.train_im_data = np.load(self.im_filenames[0])['arr_0']
        self.pose_data = np.load(self.pose_filenames[0])['arr_0']
        self.metric_data = np.load(self.label_filenames[0])['arr_0']
    ...
    def _load_and_enqueue(self):
        ...
        while not self.term_event.is_set():
            ...
            while start_i < self.train_batch_size:
                ...
                self.train_data_arr = np.load(train_data_filename)[
                                         'arr_0'].astype(np.float32)
                self.train_poses_arr = np.load(self.pose_filenames[file_num])[
                                          'arr_0'].astype(np.float32)
                self.train_label_arr = np.load(self.label_filenames[file_num])[
                                          'arr_0'].astype(np.float32)
