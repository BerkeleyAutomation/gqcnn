...
parser.add_argument('dataset_path', type=str, default=None, help='Path to dataset to calculate metrics for')
args = arge_parse.parse_args()
...
num_ims_last_file = np.load(os.path.join(dataset_path, metric_filenames[-1]))['arr_0'].shape[0]
...
for fname in metric_filenames:
    metric_data = np.load(os.path.join(dataset_path, fname))['arr_0']
